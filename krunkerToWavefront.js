...
if (process.argv.length == 3) {
	...
	//Downgrade map file by @mogjp2
	console.log("-JSON Downgrader(by Mogura#6378): Converting... new-krunker-map-json -> old-krunker-map-json");
	map.objects.forEach((object, index) => {
		var sid = object.si * 3; //si connected to xyz. xyz is not a nested array so do *3
		var s = [map.xyz[sid], map.xyz[sid+1], map.xyz[sid+2]]; // s:[width, height, depth](idk about order)
		map.objects[index].s = s; //append
	});
	console.log("-JSON Downgrader(by Mogura#6378): Done. Returning to normal krunkerToWavefront...");
